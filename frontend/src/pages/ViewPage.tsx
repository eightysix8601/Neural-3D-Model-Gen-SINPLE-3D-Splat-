 ...
 * 실시간 미리보기 포함 버전
 */
import { useEffect, useState, Suspense, useRef, lazy, useMemo } from "react";
...
import {
  ...
  CheckCircle2, XCircle, Loader2, Eye, Trash2, Bone,
} from "lucide-react";
import { getProject, deleteProject, api, assetFileUrl as buildAssetFileUrl, type Project, type Asset } from "../utils/api";
import { useProjectStore } from "../stores/useProjectStore";
import PointCloudViewer from "../components/viewer/PointCloudViewer";
import ErrorBoundary from "../components/ErrorBoundary";
import { computeUprightQuaternion } from "../utils/orient";

// 에셋을 백엔드 same-origin 프록시로 스트리밍 (MinIO 직접 접근 시 CORS 로 GLB 가
// 로드 실패 → 앱 전체 암전되던 문제 해결)
// 인증 토큰을 쿼리에 자동 첨부 (GLTFLoader/<a href> 가 헤더를 못 보내서 403 났던 문제 해결)
const assetFileUrl = buildAssetFileUrl;

// ── 진행 단계 정의 (모델별 라벨) ──────────────────────────────────────────────
// 실제 파이프라인 progress: 배경분리(5~18) → COLMAP(32~55) → 재구성학습(57~95) → 마무리(95~100)
function buildSteps(modelType?: string) {
  const m = (modelType || "sugar").toLowerCase();
  const trainLabel =
    m === "nerf" ? "NeRF 학습 (nerfacto)" :
    m === "gof"  ? "GOF 3D 학습" :
                   "3D 학습 (3DGS)";
  const meshLabel =
    m === "nerf" ? "메쉬 추출 (NeRF · Poisson)" :
    m === "gof"  ? "메쉬 추출 (GOF)" :
                   "메쉬 추출 (SuGaR)";
  return [
    { label: "배경 분리 (BiRefNet)",      range: [0,  30] },
    { label: "카메라 포즈 추정 (COLMAP)", range: [30, 55] },
    { label: trainLabel,                  range: [55, 92] },
    { label: meshLabel,                   range: [92, 100] },
  ];
}
...
function GlbModel({ url }: { url: string }) {
  ...
  // 형상으로 up축 추정 → 똑바로 세우는 회전 (Center 가 회전 후 bbox 로 재중심 잡음)
  const quat = useMemo(() => {
    const pts: number[] = [];
    ...
    return computeUprightQuaternion(pts);
  }, [scene]);
  useEffect(() => {
    scene.traverse((c) => {
...
// ── 진행률 패널 ───────────────────────────────────────────────────────────────
function ProgressPanel({ progress, step, status, error, modelType }: {
  progress: number; step: string; status: string; error?: string | null; modelType?: string;
}) {
  const STEPS = buildSteps(modelType);
  return (
...
function AssetsPanel({ assets, projectId }: { assets: Asset[]; projectId: string }) {
  ...
  const META: Record<string, { icon: string; desc: string; color: string }> = {
    ...
    obj: { icon: "📦", desc: "OBJ + 텍스처 (ZIP)",   color: "text-blue-400"   },
    ply: { icon: "🔵", desc: "포인트클라우드",        color: "text-teal-400"   },
  };

  const getFilename = (asset: Asset) => {
    if (asset.format === "obj" && asset.has_texture) return "model_with_texture.zip";
    if (asset.format === "obj") return "model.obj";
    return `model.${asset.format}`;
  };
  ...
  const download = (asset: Asset) => {
    try {
      ...
      a.href = assetFileUrl(projectId, asset.id, true);  // same-origin, attachment
      a.download = getFilename(asset);
      document.body.appendChild(a);
      a.click();
      a.remove();
      toast.success(`${asset.format.toUpperCase()} 다운로드 시작`);
    } catch {
      toast.error("다운로드 실패");
    } finally {
...
export default function ViewPage() {
  ...
  const handleDelete = async () => {
    if (!id || !confirm("이 프로젝트를 삭제하시겠습니까?")) return;
    try {
  ...
  // 완성 결과 뷰 전환: GLB(메쉬) ↔ PLY(포인트클라우드)
  const [resultView, setResultView] = useState<"glb" | "ply">("glb");
  const plyAsset = useMemo(
    () => project?.assets?.find((a) => a.format === "ply"),
    [project],
  );
  const plyAssetUrl = useMemo(
    () => plyAsset && project ? assetFileUrl(project.id, plyAsset.id) : null,
    [plyAsset, project],
  );
  ...
  useEffect(() => {
    ...
    getProject(id).then((p) => {
      ...
      if (p.status === "SUCCESS") {
        ...
        if (glb) setGlbUrl(assetFileUrl(id, glb.id));
        fetchPreview(id);
      }
  ...
  const showGlb = isSuccess && glbUrl && resultView === "glb";
  const showResultPly = isSuccess && plyAssetUrl && resultView === "ply";
  const showLivePreview = !isSuccess && preview?.type === "pointcloud" && preview.url;
  const showPointCloud = showResultPly || showLivePreview;
      ...
      <div className="h-12 border-b border-zinc-800 flex items-center px-4 gap-3 shrink-0 bg-zinc-950/80 backdrop-blur-xl">
        ...
        <div className="ml-auto flex items-center gap-1">
          {project.status === "SUCCESS" && (
            <button onClick={() => navigate(`/rigging/${id}`)}
              className="text-xs text-fuchsia-300 hover:text-fuchsia-200 transition-colors px-2.5 py-1 rounded bg-fuchsia-950/40 border border-fuchsia-800/50 hover:bg-fuchsia-900/50 flex items-center gap-1">
              <Bone className="w-3.5 h-3.5" />리깅하기
            </button>
          )}
          <button onClick={handleDelete}
            className="text-xs text-zinc-600 hover:text-red-400 transition-colors px-2 py-1 rounded hover:bg-zinc-800 flex items-center gap-1">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 p-2 min-w-0">
          <div className="w-full h-full rounded-2xl overflow-hidden bg-zinc-900 relative">
            {/* 결과 뷰 토글 (완료 + GLB/PLY 둘 다 있을 때) */}
            {isSuccess && glbUrl && plyAssetUrl && (
              <div className="absolute top-3 right-3 z-20 flex p-0.5 bg-zinc-950/80 backdrop-blur-md border border-zinc-700 rounded-xl">
                <button
                  onClick={() => setResultView("glb")}
                  className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-all ${
                    resultView === "glb" ? "bg-violet-600 text-white" : "text-zinc-400 hover:text-zinc-200"
                  }`}
                  title="메쉬 (GLB)"
                >
                  메쉬
                ...
                <button
                  onClick={() => setResultView("ply")}
                  className={`px-3 py-1.5 text-xs font-medium rounded-lg transition-all ${
                    resultView === "ply" ? "bg-teal-600 text-white" : "text-zinc-400 hover:text-zinc-200"
                  }`}
                  title="포인트클라우드 (PLY)"
                >
                  PLY
            ...
            <ErrorBoundary resetKey={
              showGlb ? glbUrl :
              showResultPly ? plyAssetUrl :
              showLivePreview ? preview?.url :
              "idle"
            }>
              {/* GLB 완성 뷰어 */}
        ...
        {/* 사이드바 — width 애니메이션 제거: 재방문/뒤로가기 시 패널이 반쯤
            가려지거나 안 열리던 버그의 원인이었음. 고정 폭으로 안정화. */}
        {sidebarOpen && (
            <div
              className="h-full w-[290px] shrink-0 border-l border-zinc-800 bg-zinc-950 flex flex-col overflow-hidden"
            >
              {/* 탭 */}
