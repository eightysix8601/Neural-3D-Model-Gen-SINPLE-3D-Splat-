...
import { Plus, Trash2, ChevronRight, Box, Clock, CheckCircle, XCircle, Loader, Camera, Bone } from "lucide-react";
    ...
    <motion.div layout initial={{opacity:0,y:20}} animate={{opacity:1,y:0}} exit={{opacity:0,scale:0.95}} whileHover={{y:-2}}
      ...
      <div className="relative h-40 bg-zinc-800">
        ...
        {project.status === "SUCCESS" && (
          <button onClick={(e) => { e.stopPropagation(); navigate(`/rigging/${project.id}`); }}
            className="absolute top-2 right-2 flex items-center gap-1 px-2 h-7 bg-fuchsia-950/70 border border-fuchsia-700/60 rounded-lg text-xs text-fuchsia-300 hover:bg-fuchsia-900/80 opacity-0 group-hover:opacity-100 transition-all">
            <Bone className="w-3 h-3" />리깅
          </button>
        )}
...
export default function HomePage() {
  ...
  const handleDelete = async (id: string) => {
    if (!confirm("이 프로젝트를 삭제하시겠습니까?\n\n3D 모델·이미지·DB 기록이 모두 영구 삭제되며 되돌릴 수 없습니다.")) return;
      ...
      <div className="max-w-7xl mx-auto px-6 py-8">
        <div className="flex items-center justify-between mb-6">
          ...
          <button onClick={() => navigate("/create")}
            className="bg-gradient-to-r from-violet-600 to-blue-600 hover:from-violet-500 hover:to-blue-500 text-white font-medium px-4 py-2 rounded-xl text-sm flex items-center gap-1.5 transition-all shadow-lg shadow-violet-900/30">
