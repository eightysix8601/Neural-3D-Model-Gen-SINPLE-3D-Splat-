...
def _unload_model():
    """BiRefNet 을 VRAM 에서 완전히 내려 SuGaR/NeRF 단계가 GPU 메모리를 확보하게 한다.

    bg_removal·sugar·nerf·gof 워커는 같은 물리 GPU(16GB)를 공유한다. 모델을
    전역으로 들고 있으면 ~2GB 가 파이프라인 내내 묶여 SuGaR refine 단계에서
    'CUDA error: device not ready'(사실상 OOM)로 죽는다. 배치가 끝나면 내린다.
    """
    global _model
    if _model is None:
        return
    try:
        ...
        logger.info("[BiRefNet] 모델 언로드 및 VRAM 해제 완료")
    except Exception as e:
        logger.warning(f"[BiRefNet] 언로드 중 경고(무시): {e}")
        _model = None
